...
```{r 2toomany,echo=F, warning=FALSE}
set.seed(42)
felicidad<-rnorm(500,100,500)
knitr::kable(matrix(round(felicidad),ncol=10,nrow=50))
```
...
Hacer un histograma será nuestro primer acto de resumir oficialmente algo sobre los datos. Ya no vamos a mirar cada punto individualmente, sino que vamos a ver cómo se agrupan los números. Mirá la @fig-happyHist, un histograma de los datos de felicidad, y después lo explicamos.

```{r, echo=F, warning=FALSE}
#| label: fig-happyHist
#| fig-cap: "Un histograma de las puntuaciones de felicidad"
hist(felicidad,ylab = "frecuencia",main="Histograma de la variable felicidad",breaks = 
       seq(from = -1500, to = 2000, by = 500))
...
¿Qué nos ayuda a ver el histograma sobre los datos? Primero, podemos ver la **forma** de los datos. La forma del histograma se refiere a cómo sube y baja. Nos dice dónde están concentrados los datos. Por ejemplo, cuando las barras son bajas, sabemos que no hay muchos datos ahí. Cuando son altas, sabemos que hay más. Entonces, ¿dónde está la mayoría de los datos? Parece que están sobre todo en los dos bins del medio, entre -500 y 500. También podemos ver el **rango** de los datos, o sea los mínimos y máximos. La mayoría de los datos están entre -1500 y +1500, así que no hay felicidad ni tristeza infinitas en este conjunto de datos.
...
g1 <- qplot(x=felicidad, color="white", bins=5)+
  theme_classic()+
  theme(legend.position="none")

g2 <- qplot(x=felicidad, color="white", bins=10)+
  theme_classic()+
  theme(legend.position="none")

g3 <- qplot(x=felicidad, color="white", bins=20)+
  theme_classic()+
  theme(legend.position="none")

g4 <- qplot(x=felicidad, color="white", bins=50)+
  theme_classic()+
  theme(legend.position="none")
...
## Cuantiles, cuartiles y percentiles: cortando la torta de los datos

Antes de meternos con los **diagramas de caja**, necesitamos conocer una idea básica pero poderosa: **cómo dividir los datos ordenaditos en partes**. Para eso están los **cuantiles**.

### ¿Qué son los cuantiles?

Imaginate que tenés una lista de datos, como por ejemplo puntajes en un test, y los ordenás de menor a mayor. Los cuantiles son como cortes que hacés en esa lista para dividirla en partes iguales. Según en cuántas partes la quieras dividir, cambian de nombre:

-   Si la dividís en 4 partes → **cuartiles**
-   Si la dividís en 10 → **deciles**
-   Si la dividís en 100 → **percentiles**

Cada uno de esos cortes nos ayuda a entender cómo están repartidos los datos. ¿Están todos apretados en un extremo? ¿Se reparten parejito? ¿Hay alguno que se va al pasto? Los cuantiles te dan pistas.

### Percentiles: lo escuchaste desde bebé

Probablemente la primera vez que escuchaste hablar de **percentiles** fue cuando eras un ser humano de apenas unos kilos. Alguna vez alguien te dijo (o escuchaste que le dijeron a un bebé):

> "Está en el percentil 99 de peso"\
> o\
> "Uy, está medio flaquito, está en el percentil 3 de talla"

¿Eso qué quiere decir?\
Que si estabas en el **percentil 99**, pesabas más que el 99% de bebés de tu edad.\
Y si estabas en el **percentil 1**, solo el 1% pesaba menos que vos.

¡Así de simple! Los percentiles te dicen **qué porcentaje de la población está por debajo tuyo**.

### ¿Y los cuartiles?

Bueno, los **cuartiles** son un caso especial de percentiles:

-   **Primer cuartil (Q1)** = percentil 25\
-   **Segundo cuartil (Q2)** = percentil 50 (también conocido como la **mediana**)\
-   **Tercer cuartil (Q3)** = percentil 75

Es decir, el Q1 es el valor debajo del cual cae el 25% de los datos. El Q3 es el punto debajo del cual cae el 75%. Entre Q1 y Q3 está la mitad de tus datos: eso se llama el **rango intercuartílico** (pero tranqui, ya lo vamos a ver con un dibujito).
...
Al mirar este gráfico, esto es lo que deberías interpretar: la línea gruesa en el medio de la caja es la mediana; la caja en sí abarca desde el percentil 25 hasta el 75; y los "bigotes" cubren todo el rango desde el valor mínimo hasta el máximo. Puedes observarlo en la @fig-boxplotanotado.

```{r, echo=F, warning=FALSE}
#| label: fig-boxplotanotado
#| fig-cap: "Boxplot con anotaciones"
knitr::include_graphics(here::here("imgs", "boxplot1_anotado_es.png"))
```
...
```{r, echo=F, warning=FALSE}
resumen <- summary(felicidad) 
names(resumen) <- c("Mínimo", "Perc. 25", "Mediana", "Media", "Perc. 75", "Máximo")

print(resumen)
```

```{r, echo=F, warning=FALSE}
#| label: fig-boxplotfelicidad
#| fig-cap: "Boxplot - Felicidad"

boxplot(felicidad)
```

### Uso de diagramas de caja para detectar valores atípicos
...
```{r 2diffscores, echo=F, warning=FALSE}
...
```{r 2diffscoreSmall,echo=F, warning=FALSE}
numbers<-c(1, 2,3)
...
#### Desviaciones respecto de la media. Puntajes de diferencia con respecto a la media
...
```{r 2deviationssquared,echo=F, warning=FALSE}
medidas<-1:6
valores<-c(1,6,4,2,6,8)
mean_scores<-mean(valores)
Diferencia_a_la_media<-valores-mean_scores
...
```{r 2absolute, echo=F, warning=FALSE}
medidas<-1:6
valores<-c(1,6,4,2,6,8)
mean_scores<-mean(valores)
Diferencia_a_la_media<-valores-mean_scores
...
the_df %>% rbind(c("Sumas",colSums(the_df[1:6,2:5]))) %>%
  rbind(c("Medias",colMeans(the_df[1:6,2:5]))) %>% knitr::kable(align="c")
...
Para ilustrar este punto ---y prepararte para los temas del próximo capítulo--- mirá este ejemplo. Se llama el Cuarteto de Anscombe, porque estos gráficos y valores tan curiosos fueron producidos por \[Anscombe, 1973\]. En la @fig-anscombe estás viendo pares de mediciones. Cada gráfico tiene un eje X y un eje Y, y cada punto representa dos valores. A simple vista, cada gráfico se ve muy distinto, ¿no?
...
knitr::kable(ac_long_summary,col.names = c("cuarteto","media x", "varianza x","media y","varianza y"),align = "c")
```
