...
```{r echo=F, eval=F}
# Esta función genera medias muestrales a partir de múltiples muestras
...
# Creamos un data frame para guardar todo
all_df <- data.frame()

# Simulamos para distintos tamaños de muestra y múltiples repeticiones
for (sims in 1:10) {
...
Perfecto, acá va la traducción del código original que genera la animación, con los comentarios en español:
...
Perfecto, acá sigue la traducción del bloque de código y del texto que le sigue:
...
ggplot(all_df, aes(x = sample)) +
  geom_histogram(aes(y = (..density..) / max(..density..)^.8),
                 color = "white", fill = "grey") +
  geom_histogram(aes(x = sample_means, y = (..density..) / max(..density..)),
                 fill = "blue", color = "white", alpha = .5) +
...
Tenés razón, gracias por señalarlo. Acá va la traducción fiel del código de la figura `fig-4normalSDspercentsB` con comentarios en español:
