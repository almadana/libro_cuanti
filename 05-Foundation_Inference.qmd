f--- author: - Matthew J. C. Crump lang: es abstract-title: Notas abstract: Traducido al español rioplatense por ChatGPT4-o bajo la supervisión de Álvaro Cabana. aliases: \[foundations-for-inference.html\] ---
...
Como recordatorio del capítulo anterior, la @fig-5unifagain muestra que la forma de una distribución uniforme es completamente plana.
...
hist(a,main = "Histograma",xlab = "número",ylab="frecuencia")
```
...
Para tener una mejor idea de la variabilidad muestral, repitamos ese proceso **diez veces**. La @fig-5manysamples muestra 10 histogramas, cada uno representando una muestra distinta de 20 números:
...
Así que aumentemos el número de observaciones en cada muestra, de 20 a 100. Vamos a crear 10 muestras (cada una con 100 observaciones), y a graficar un histograma para cada una. Todas estas muestras se extraen de la **misma** distribución uniforme. Esto significa que deberíamos esperar que **cada número del 1 al 10 ocurra unas 10 veces** en cada muestra. Los histogramas están en la @fig-5unifsamp100.
...
#| fig-cap: "Histogramas de diferentes muestras de una distribución uniforme. N = 1000 para cada muestra."
#| out-width: "100%"

a <- sample(1:10, 1000 * 10, replace = TRUE)
...
Lo primero que vamos a hacer es simular que realizás el experimento de las bolitas **10 veces seguidas**. Eso va a producir 10 conjuntos distintos de resultados. La @fig-5bolitasims muestra gráficos de barras para cada repetición del experimento. Ahora podemos ver si la mano izquierda sacó más bolitas verdes que la derecha.
...
ggplot(plot_df, aes(x = diferencia)) +
  ...
  theme_classic() +
  ggtitle("Histograma de diferencias de medias entre dos muestras (n = 20)\nambas extraídas de la misma distribución normal (μ = 100, σ = 15)") +
...
Se ve así en la @fig-5crumpdecision:
...
La @fig-5crumpuncertainty muestra **mi** opinión sobre dónde se ubican las zonas grises:
...
Así que vamos a ilustrar **otro tipo de estrategia** para tomar decisiones. Recién hablamos de una que incluía líneas y regiones. Eso hace que parezca una elección binaria: podemos **descartar** o **no descartar** el papel del azar. Otra perspectiva es que **todo es un matiz de gris**, como en la @fig-5crumpshade.
...
ggplot(all_df, aes(x = sample)) +
  ...
  theme_classic() + ggtitle("Distribución muestral de diferencias de media, según N") +
  ylab("conteo") +
  xlab("Diferencias entre medias")
...
La @tbl-5minmax muestra los valores mínimos y máximos que el azar produjo para cada uno de los cuatro tamaños muestrales:
...
the_range <- all_df %>%
  ...
  summarize(menor = min(sample),
            mayor = max(sample))

knitr::kable(the_range,col.names = c("Tamaño de muestra","menor","mayor"),align = 'c',digits = 2)
```
...
Veamos una pequeña animación de lo que ocurre durante la prueba de aleatorización. La @fig-5randtest muestra datos de otro experimento ficticio, pero los principios son los mismos. Después volveremos al experimento del chicle.
...
Este es el histograma de las diferencias de medias obtenidas con la prueba de aleatorización. Para esta simulación, aleatorizamos los resultados del experimento original 1000 veces. Esto es lo que **podría haber pasado**. La línea azul en la @fig-5randhist muestra **dónde se ubica la diferencia observada** sobre el eje x.
