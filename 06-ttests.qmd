...
error_muestral <- length(1000)

for (i in 1:1000) {
  s <- rnorm(10, 0, 1)
  media_muestral[i] <- mean(s)
  error_muestral[i] <- sd(s) / sqrt(length(s))
}

plot(media_muestral, error_muestral)
```
...
hist(media_muestral/error_muestral, breaks=30,ylab="Frecuencia", main="Histograma de media_muestral/error_muestral")
```
...
Todos completan el test, corregimos, y calculamos el desempeño (porcentaje de respuestas correctas). Queremos saber si estuvieron adivinando. Si fue así, el promedio debería rondar el 50%, que sería igual a azar. Veamos la @tbl-tsmall.
...
Volvamos al capítulo sobre muestreo y distribuciones. Ahí hablamos de la distribución muestral de la media. Generamos muchas muestras, calculamos la media de cada una, y armamos un histograma de esas medias. Más adelante dijimos que podíamos hacer eso con **cualquier** estadístico. Para cada muestra podíamos calcular la media, la desviación estándar, el error estándar, e incluso *t*. Podríamos generar 10.000 muestras y hacer cuatro histogramas, uno para cada distribución muestral de cada estadístico. Eso fue exactamente lo que hice, y los resultados están en los cuatro paneles de la @fig-6sampdists. Usé un tamaño de muestra de 20, y tomé observaciones aleatorias de una distribución normal, con media = 0, y la desviación estándar = 1. Veamos las distribuciones muestrales de cada estadístico. *t* se calculó asumiendo que la media poblacional es 0.
...
Acá va una forma rápida de reportar los resultados de la prueba t: t(4) = .72, p = .509.
...
### Test unilateral (de una cola, o one-tailed en inglés)
...
### Test bilateral (de dos cola, o two-tailed en inglés)
...
@fig-6twotailedt muestra cuáles son el 5% más extremo de los valores $t$ cuando ignoramos su signo (ya sean positivos o negativos).
...
## Prueba t de muestras independientes: ¿el regreso d la prueba t?
...
hist(guarda_ts,main="Histograma de guarda_ts",xlab="valores de guarda_ts",ylab="Frecuencia")
```
...
hist(guarda_ps,main="Histograma de guarda_ps",xlab="valores de guarda_ps",ylab="Frecuencia")
```
